...
# region CLASS_TagHeaderPattern [DOMAIN(DocumentProcessing): ...; CONCEPT(PatternMatching): ...; TECH(Regexp): ...]
## @purpose TagHeaderPattern for document structure extraction pipeline
class TagHeaderPattern(TagPattern):
    ...
    **if level_2, tag_hierarchy_level.level_2, default_level_2 are empty**.
    ...
    def __init__(self,
        ...
        self.logger.debug(f"[IMP:4][TagHeaderPattern][__init___INIT] Starting")
    ...
    def match(self, line: LineWithMeta) -> bool:
        self.logger.debug(f"[IMP:4][TagHeaderPattern][match_INIT] Starting")
        """
    ...
    def get_hierarchy_level(self, line: LineWithMeta) -> HierarchyLevel:
        self.logger.debug(f"[IMP:4][TagHeaderPattern][get_hierarchy_level_INIT] Starting")
...
# endregion CLASS_TagHeaderPattern
# region MODULE_CONTRACT [DOMAIN(DocumentProcessing): ...; CONCEPT(PatternMatching): ...; TECH(Regexp): ...]
## @modulecontract
## @purpose Document structure extraction for structure_extractors/patterns/tag_header_pattern: line classification, hierarchy level assignment, pattern matching.
## @scope Structure extraction pipeline — structure_extractors/patterns/tag_header_pattern
...
## @modulemap
## CLASS [Weight 7][Structure extraction] => TagHeaderPattern
## @usecases
## - Extract structure: Reader → StructureExtractor → HierarchyBuilder → AnnotatedDocument
def _module_contract():
    pass
# endregion MODULE_CONTRACT
# GREP_SUMMARY: structure extractors, patterns, tag header pattern
# STRUCTURE: ▶ structure_extractors/patterns/tag_header_pattern → ○ TagHeaderPattern.cls → ⎋ result